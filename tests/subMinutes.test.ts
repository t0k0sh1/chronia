...
/**
 * Test Design for subMinutes
 *
 * Function signature: subMinutes(date: Date | number, amount: number): Date
 *
 * Equivalence Partitioning:
 * - Class 1: Valid Date + positive amount → Returns new Date with minutes subtracted
 * - Class 2: Valid Date + negative amount → Returns new Date with minutes added
 * - Class 3: Valid Date + zero amount → Returns same date
 * - Class 4: Valid timestamp + valid amount → Returns new Date with minutes subtracted
 ...
 * - Hour boundary backward: 13:15 - 30 minutes → 12:45
 * - Day boundary backward: 00:15 - 30 minutes → 23:45 previous day
 * - Month boundary backward: Feb 1 00:15 - 30 minutes → Jan 31 23:45
 * - Year boundary backward: Jan 1 00:15 - 30 minutes → Dec 31 23:45 previous year
 * - Fractional amounts: Truncation behavior (floor)
 * - Large values: Subtracting many minutes
...
describe("subMinutes", () => {
  describe("Equivalence Class 1: Valid Date + positive amount", () => {
    it("should subtract positive minutes within same hour", () => {
      // Arrange
      const date = new Date(2025, 0, 15, 12, 45, 0);
      const amount = 15;

      // Act
      const result = subMinutes(date, amount);
    ...
    it("should subtract large number of minutes", () => {
      // Arrange
  ...
  describe("Equivalence Class 2: Valid Date + negative amount", () => {
    it("should add minutes when amount is negative", () => {
      // Arrange
      const date = new Date(2025, 0, 15, 12, 30, 0);
      const amount = -15;
  ...
  describe("Equivalence Class 3: Valid Date + zero amount", () => {
    it("should return same date when subtracting zero minutes", () => {
      // Arrange
  ...
  describe("Equivalence Class 4: Valid timestamp + valid amount", () => {
    it("should accept timestamp input and subtract minutes", () => {
      // Arrange
      const date = new Date(2025, 0, 15, 12, 45, 0);
      ...
      // Act
      const result = subMinutes(timestamp, amount);
    ...
    it("should accept timestamp and add minutes (negative amount)", () => {
      // Arrange
      const timestamp = new Date(2025, 0, 15, 12, 15, 0).getTime();
      const amount = -30;
  ...
  describe("Equivalence Class 5: Invalid Date + valid amount", () => {
    it("should return Invalid Date when base date is invalid", () => {
      ...
      // Act
      const result = subMinutes(invalidDate, amount);
  ...
  describe("Equivalence Class 7: Boundary crossing - Hour boundaries", () => {
    it("should cross hour boundary backward (13:15 - 30 minutes → 12:45)", () => {
      // Arrange
      const date = new Date(2025, 0, 15, 13, 15, 0);
      const amount = 30;
  ...
  describe("Equivalence Class 7: Boundary crossing - Day boundaries", () => {
    it("should cross day boundary backward (00:15 - 30 minutes → 23:45 previous day)", () => {
      // Arrange
      const date = new Date(2025, 0, 2, 0, 15, 0);
      const amount = 30;
    ...
    it("should cross day boundary forward (23:45 - (-30) minutes → 00:15 next day)", () => {
      // Arrange
      const date = new Date(2025, 0, 1, 23, 45, 0);
      const amount = -30;
  ...
  describe("Equivalence Class 7: Boundary crossing - Month/Year boundaries", () => {
    it("should cross month boundary backward (Feb 1 00:15 - 30 minutes → Jan 31 23:45)", () => {
      // Arrange
      const date = new Date(2025, 1, 1, 0, 15, 0);
      const amount = 30;
    ...
    it("should cross year boundary backward (Jan 1 00:15 - 30 minutes → Dec 31 23:45)", () => {
      // Arrange
      const date = new Date(2025, 0, 1, 0, 15, 0);
      const amount = 30;
  ...
  describe("Boundary Value Analysis: Fractional amounts", () => {
    it("should truncate fractional amount (positive)", () => {
      ...
      // Assert
      expect(result.getMinutes()).toBe(29); // floor(1.9) = 1 minute subtracted
    });
    ...
    it("should truncate fractional amount (negative)", () => {
      ...
      // Assert
      expect(result.getMinutes()).toBe(1); // floor(-1.9) = -1 minute (adds 1)
    });
  });
  ...
  describe("Boundary Value Analysis: Preserve lower time units", () => {
    it("should preserve seconds and milliseconds", () => {
      // Arrange
      const date = new Date(2025, 0, 15, 12, 45, 45, 123);
      const amount = 15;
