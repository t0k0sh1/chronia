...
function getOverdueMilestones(
  ...
  return milestones.filter(
    (m) => !m.completed && isAfterOrEqual(asOf, m.dueDate),
  );
}
...
const logs: LogEntry[] = [
  {
    timestamp: new Date(2025, 0, 10, 9, 0),
  ...
  {
    timestamp: new Date(2025, 0, 20, 11, 0),
